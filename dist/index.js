...
app.post('/generate-music-audio', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    console.log(req.body);
    const tempFilePath = path.join('./uploads', 'temp_output.mp3');
    const { fileInput, outputFile, clarityValue, speedValue, backgroundMusic } = req.body;
    const inputPath = fileInput;
    let tagData = 'uploads/jazz.mp3';
    if (tagFileMap.get(backgroundMusic) !== undefined) {
        tagData = (_a = tagFileMap.get(backgroundMusic)) !== null && _a !== void 0 ? _a : 'uploads/jazz.mp3';
        console.log(tagData);
    }
    ...
    try {
        ...
        const responseUploadAudio = yield axios.post('https://ojpwyux64l.execute-api.us-east-1.amazonaws.com/default/uploadMergedAudio', { fileName: outputFile, fileType: 'audio' }, {
            headers: {
        ...
        const responsePutS3object = yield axios.put(presignedUrlOutput, audioBuffer, {
            headers: {
        ...
        const responseFetchPresignedUrl = yield axios.post('https://i6xasb8wxd.execute-api.us-east-1.amazonaws.com/default/fetchAudioFilePresignedUrl', { fileName: outputFile }, {
            headers: {
                'Content-Type': 'application/json',
            }
        });
        const data = yield responseFetchPresignedUrl.data;
